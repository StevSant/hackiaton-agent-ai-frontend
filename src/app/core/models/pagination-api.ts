...
export function mapToPaginated<T>(data: any, params?: { page?: number; limit?: number }): Paginated<T> {
  ...
  if (typeof data?.total === 'number' && Array.isArray(data?.items)) {
    ...
    const page = params?.page ?? (limit ? Math.floor((data.offset ?? 0) / limit) + 1 : 1);
    return { items: data.items as T[], page, limit, total };
  }
  // Ultimate fallback: empty page
  return { items: [], page: params?.page ?? 1, limit: params?.limit ?? 0, total: 0 };
}
